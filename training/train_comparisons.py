...
def main():
    ...
    parser.add_argument('--model', required=True, choices=['srgan', 'aga_gan', 'wipa'])
    args = parser.parse_args()
    cfg = load_config(args.config)
    set_seed(cfg.get('seed', 42))
    dcfg = cfg['dataset']
    tcfg = cfg['train']
    model = build_model(args.model, {'target_size': dcfg['image_size'], 'base_channels': cfg['model']['generator_base_channels']})
